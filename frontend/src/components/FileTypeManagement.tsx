...
const FileTypeManagement: React.FC = () => {
  ...
  const fetchSystemSummary = async () => {
    try {
      const response = await adminApiClient.get('/api/file-types/system/summary');
  ...
  const fetchFileTypes = async () => {
    setLoading(true);
    try {
      const response = await adminApiClient.get('/api/file-types');
      if (response.data.success) {
  ...
  const fetchHealthStatuses = async () => {
    try {
      const response = await adminApiClient.get('/api/file-types/health/all');
      if (response.data.success) {
  ...
  const saveFileType = async (values: any) => {
    try {
      ...
      if (editingType) {
        // 更新
        await adminApiClient.put(`/api/file-types/${editingType.file_type}`, payload);
        message.success('文件类型更新成功');
      } else {
        // 新增
        await adminApiClient.post('/api/file-types', payload);
  ...
  const deleteFileType = async (fileType: string) => {
    try {
      await adminApiClient.delete(`/api/file-types/${fileType}`);
  ...
  const toggleEnabled = async (fileType: string, enabled: boolean) => {
    try {
      await adminApiClient.patch(`/api/file-types/${fileType}/toggle`, { enabled });
  ...
  const repairFileType = async (fileType: string) => {
    try {
      await adminApiClient.post(`/api/file-types/${fileType}/repair`);
