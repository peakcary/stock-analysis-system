...
interface PaymentPackage {
  ...
  membership_type: 'free' | 'pro' | 'premium' | 'FREE' | 'PRO' | 'PREMIUM';
  description?: string;
  is_active: boolean;
  sort_order: number;
  created_at: string;
...
const PackageManagement: React.FC = () => {
  ...
  const fetchPackages = async () => {
    setLoading(true);
    try {
      const response = await adminApiClient.get('/api/admin/packages');
  ...
  const fetchStats = async () => {
    try {
      const response = await adminApiClient.get('/api/admin/packages/stats');
  ...
  const handleCreate = async (values: any) => {
    try {
      await adminApiClient.post('/api/admin/packages', values);
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'action',
      width: 200,
      render: (_: any, record: PaymentPackage) => (
        <Space>
          <Button
            type="link"
            icon={<EyeOutlined />}
