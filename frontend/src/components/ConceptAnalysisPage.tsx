...
const ConceptAnalysisPage: React.FC = () => {
  ...
  const fetchConceptSummaries = async () => {
    ...
    try {
      const response = await adminApiClient.get(
        `/stock-analysis/concepts/daily-summary?trading_date=${tradingDate}&sort_by=total_volume&sort_order=desc&size=1000`
      );
  ...
  const fetchStockInfo = async (conceptName: string) => {
    ...
    try {
      ...
      const response = await adminApiClient.get(
        `/stock-analysis/concepts/${encodeURIComponent(conceptName)}/rankings?trading_date=${tradingDate}&page=1&size=10000`
      );
