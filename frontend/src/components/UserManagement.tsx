...
const UserManagement: React.FC = () => {
  ...
  const fetchUsers = async (page = 1, pageSize = 10) => {
    ...
    try {
      ...
      // 使用新的客户端用户管理API
      const response = await adminApiClient.get('/api/admin/client-users/users', {
        params
      });
  ...
  const fetchStats = async () => {
    try {
      const response = await adminApiClient.get('/api/admin/client-users/stats');
  ...
  const fetchUserDetail = async (userId: number) => {
    try {
      // 获取用户查询记录
      const queriesResponse = await adminApiClient.get(`/api/admin/users/${userId}/queries`);
      setUserQueries(queriesResponse.data.queries || []);

      // 获取用户支付记录
      const paymentsResponse = await adminApiClient.get(`/api/admin/users/${userId}/payments`);
  ...
  const handleSaveUser = async (values: any) => {
    try {
      if (editingUser) {
        // 更新用户
        await adminApiClient.put(`/api/admin/users/${editingUser.id}`, values);
        message.success('用户更新成功');
      } else {
        // 创建用户
        await adminApiClient.post('/api/admin/users', values);
  ...
  const handleUpgradeMembership = async (userId: number, membershipType: string) => {
    try {
      await adminApiClient.post(`/api/admin/users/${userId}/membership`, {
  ...
  const handleResetPassword = async (userId: number) => {
    ...
    try {
      await adminApiClient.post(`/api/admin/users/${userId}/reset-password?new_password=${newPassword}`, {});
