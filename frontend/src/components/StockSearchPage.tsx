...
import { clientApiClient } from '../../../shared/client-auth';
...
const StockSearchPage: React.FC = () => {
  ...
  const handleSearch = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await clientApiClient.get(`/api/v1/stocks/${searchValue.trim().toUpperCase()}`);
  ...
  const fetchConceptStocks = useCallback(async (conceptName: string) => {
    ...
    try {
      const response = await clientApiClient.get(`/api/v1/concepts/${encodeURIComponent(conceptName)}/stocks`);
