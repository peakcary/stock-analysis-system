...
const EeeImportRecords: React.FC<EeeImportRecordsProps> = ({ refreshTrigger }) => {
  ...
  const fetchRecords = async (page: number = 1, pageSize: number = 20, tradingDate?: string) => {
    ...
    try {
      ...
      const url = new URL(`/api/universal-import/eee/records`, window.location.origin);
  ...
  const handleRecalculate = async (tradingDate: string) => {
    ...
    try {
      const response = await adminApiClient.post(`/api/universal-import/eee/recalculate?trading_date=${tradingDate}`, {}, {
        timeout: 180000
      });
