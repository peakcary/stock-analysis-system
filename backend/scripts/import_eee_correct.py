"""
EEE 热度数据专用导入脚本 (修正版)
将 EEE.txt 文件中的热度数据批量导入到 eee_daily_trading 表
"""
...
def parse_eee_file(file_path: str):
    """
    解析 EEE.txt 文件，按日期分组返回数据
    格式: 股票代码[Tab]交易日期[Tab]热度值
    例如: SH110062\t2024-02-20\t33082.000000
    """
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                ...
                try:
                    ...
                    heat_value = int(float(parts[2].strip()))  # 转为整数
...
def insert_eee_data(db: Session, data_by_date: dict, file_path: str, file_hash: str):
    """
    将 EEE 数据插入到 eee_daily_trading 表
    """
    from app.models.stock import EeeDailyTrading, EeeImportRecord
    ...
    for trade_date_str, records in sorted(data_by_date.items()):
        try:
            ...
            # 检查是否已存在该日期的数据
            existing_count = db.query(EeeDailyTrading).filter(
                EeeDailyTrading.trading_date == trade_date
            ).count()
            ...
            for record in records:
                eee_data = EeeDailyTrading(
                    ...
                    trading_volume=record['heat_value']
                )
                db.add(eee_data)
            ...
            logger.info(f"  ✓ {trade_date_str}: 插入 {len(records)} 条记录到 eee_daily_trading")
    ...
    try:
        import_record = EeeImportRecord(
            ...
            error_records=sum(len(v) for v in data_by_date.values() if datetime.strptime(list(data_by_date.keys())[list(data_by_date.values()).index(v)], '%Y-%m-%d').date() in [datetime.strptime(d, '%Y-%m-%d').date() for d in failed_dates]),
    ...
    logger.info(f"\n总计插入 {total_inserted} 条 EEE 数据到 eee_daily_trading\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='EEE 热度数据导入脚本 (修正版)')
    parser.add_argument('--file', type=str, default='/Users/peakom/Downloads/EEE.txt',
                        help='EEE.txt 文件路径')
...
""")
    ...
    try:
        ...
        data_by_date = parse_eee_file(args.file)
        ...
        with Session(engine) as db:
            successful, failed = insert_eee_data(db, data_by_date, args.file, file_hash)
