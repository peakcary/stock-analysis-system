...
const ConvertibleBondPage: React.FC = () => {
  ...
  const searchConvertibleBonds = async () => {
    ...
    try {
      ...
      const response = await fetch(`/stock-analysis/convertible-bonds/concepts?trading_date=${tradingDate}&limit=${limit}`, {
        headers: {
  ...
  const viewBondChart = async (bondCode: string, conceptName: string) => {
    try {
      const token = localStorage.getItem('admin_token') || localStorage.getItem('token');
      const response = await fetch(`/stock-analysis/stock/${bondCode}/chart-data?concept_name=${conceptName}&days=30`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
