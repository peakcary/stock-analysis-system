...
const EnhancedPaymentModal: React.FC<EnhancedPaymentModalProps> = ({
  ...
  const createPaymentOrder = async (method: 'wechat_native' | 'wechat_h5') => {
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        startPolling(response.data.data.out_trade_no);
      } else {
        message.error(response.data?.message || '创建订单失败');
      }
    } catch (error: any) {
      console.error('创建订单失败:', error);
      message.error(error.response?.data?.detail || '创建订单失败，请重试');
    } finally {
      setLoading(false);
    }
  };
  ...
  const startPolling = (outTradeNo: string) => {
    const interval = setInterval(async () => {
      try {
        const response = await paymentAPI.getOrderStatus(outTradeNo);
  ...
  const cancelOrder = async () => {
    ...
    try {
      const response = await paymentAPI.cancelOrder(paymentOrder.out_trade_no);
  ...
  const simulatePaymentSuccess = async () => {
    ...
    try {
      const response = await paymentAPI.mockComplete(paymentOrder.out_trade_no);
