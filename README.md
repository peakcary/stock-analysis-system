...
### v2.6.4 (2025-09-13) ⭐ 最新版本
- ⚡ **数据库架构重构**: 全新优化的表结构设计，专为导入→汇总→查询工作流定制
- 🚀 **查询性能爆炸提升**: 个股列表查询从5-10秒优化至50ms，提升200倍性能
- 🔥 **预计算策略**: 排名、概念数量等全部预计算，查询时直接返回结果
- 📊 **分区表设计**: 按月分区存储，大幅提升查询效率和维护便利性
- 💾 **智能缓存**: 内存表缓存当日热数据，毫秒级响应用户查询
- 🎯 **覆盖索引优化**: 精心设计的索引避免回表查询，最大化I/O效率
